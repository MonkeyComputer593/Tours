            ...
            >
              <div className="p-6 md:p-8">
                <div className="flex justify-between items-center mb-6 md:mb-8">
                  <div>
                    <h3 className="text-2xl md:text-3xl font-black text-gray-900">{t('contact.title')}</h3>
                    <p className="text-gray-500 font-medium text-sm">{t('contact.subtitle')}</p>
                  </div>
                  <button 
                    onClick={() => setIsOpen(false)}
                ...
                {isSent ? (
                  <div className="text-center py-8 md:py-12">
                    ...
                    <h4 className="text-xl md:text-2xl font-black text-gray-900 mb-2">{isEnglish ? 'Message Sent!' : '¡Mensaje Enviado!'}</h4>
                    <p className="text-gray-500 text-sm">{isEnglish ? 'We will contact you soon via WhatsApp' : 'Te contactaremos pronto por WhatsApp'}</p>
                  </div>
                ) : (
                  <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                    <div>
                      <label className="block text-[10px] uppercase font-black tracking-widest text-gray-400 mb-2">{t('contact.name')}</label>
                      <input 
                        name="name"
                        type="text" 
                        required
                    ...
                    <div>
                      <label className="block text-[10px] uppercase font-black tracking-widest text-gray-400 mb-2">{t('contact.email')}</label>
                      <input 
                        name="email"
                        type="email" 
                        required
                    ...
                    <div>
                      <label className="block text-[10px] uppercase font-black tracking-widest text-gray-400 mb-2">{t('contact.message')}</label>
                      <textarea 
                        name="message"
                        required
                        rows={3}
                ...
                <div className="mt-6 md:pt-6 border-t border-gray-100 flex items-center justify-center gap-4">
                  ...
                  <div className="text-left">
                    <p className="text-[10px] uppercase font-black tracking-widest text-gray-400">{t('contact.callUs')}</p>
                    <p className="text-base md:text-lg font-black text-gray-900">+593 96 190 6731</p>
                  </div>
                </div>
              </div>
            </motion.div>

      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-12">
        ...
        >
          ...
          <h2 className="text-3xl lg:text-6xl font-black text-gray-900 tracking-tighter leading-none uppercase mt-3 lg:mt-4">
            {isEnglish ? 'We Are' : 'Estamos'} <span className="text-gray-300">{isEnglish ? 'Here' : 'Aquí'}</span>
          </h2>
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              >
                ...
                <span className="text-xs font-bold uppercase tracking-wider text-gray-600 text-center">{ADDRESS}</span>
              </a>
              <a 
              ...
              >
                ...
                <span className="text-xs font-bold uppercase tracking-wider text-gray-600 text-center">{t('common.whatsapp')}</span>
              </a>
              <a 
                href={`mailto:${EMAIL}`}
                className="flex flex-col items-center p-4 bg-gray-50 rounded-xl hover:bg-orange-50 transition-colors"
              >
                ...
                <span className="text-xs font-bold uppercase tracking-wider text-gray-600 text-center">{t('common.email')}</span>
              </a>
            </div>
            ...
            {isSent ? (
              ...
              >
                ...
                <h3 className="text-xl font-black text-gray-900 mb-2">{isEnglish ? 'Message Sent!' : '¡Mensaje Enviado!'}</h3>
                <p className="text-gray-500">{isEnglish ? 'We will contact you soon via WhatsApp.' : 'Te contactaremos pronto por WhatsApp.'}</p>
              </motion.div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] uppercase font-bold tracking-widest text-gray-400 mb-2">{isEnglish ? 'Name' : 'Nombre'}</label>
                    <input 
                      type="text"
                      required
                      value={formData.name}
                  ...
                  <div>
                    <label className="block text-[10px] uppercase font-bold tracking-widest text-gray-400 mb-2">{isEnglish ? 'Phone' : 'Teléfono'}</label>
                    <input 
                      type="tel"
                      required
                ...
                <div>
                  <label className="block text-[10px] uppercase font-bold tracking-widest text-gray-400 mb-2">{t('contact.email')}</label>
                  <input 
                    type="email"
                    required
                ...
                <div>
                  <label className="block text-[10px] uppercase font-bold tracking-widest text-gray-400 mb-2">{isEnglish ? 'Tour of Interest' : 'Tour de Interés'}</label>
                  <select 
                    value={formData.tour}
                ...
                <div>
                  <label className="block text-[10px] uppercase font-bold tracking-widest text-gray-400 mb-2">{t('contact.message')}</label>
                  <textarea 
                    rows={4}
                    required
                    ...
                    placeholder={isEnglish ? 'How can we help you?' : '¿En qué podemos ayudarte?'}
                  />
                </div>
